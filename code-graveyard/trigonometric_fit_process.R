...
#'
#' \code{trigonometric_fit_process} Returns a list of fitted and predicted values of periodically repeating data using sine and cosine functions.  This works over a list of data sites efficiently processing multiple locations
#'
...
trigonometric_fit_process <- function(data,formulas) {
  ...
  models_val<-invoke_map(over_list,.f=function(x,y){ trigonometric_fit(x,y)$fit })
  ...
  # Compute the model fit
  fit_results_val<-invoke_map(over_list,.f=function(x,y){ trigonometric_fit(x,y)$prediction })
