...
#'
#' \code{periodic_fit_sample} Returns fitted and predicted values of periodically repeating data where a certain percentage of the data have been excluded in the fit.
#'
...
periodic_fit_sample <- function(data_in,regression_formula,percentage)
{
  ...
  model_stats_vals <- fitResults %>% glance %>%
    select(r.squared,df,logLik,AIC,BIC) %>%
    mutate(pct = percentage)
  ...
  return(list("prediction"=newResults,"fit"=fitResults,"model_stats"=model_stats_vals))
}
