...
# Split the albedo data up by site
data <- mcd43A3 %>% split(.$site)

# Add the data values onto each of the fitted results
model_w_data<-mcd43A3_results$fitted  %>%
split(.$model) %>%
  map2_df(list(mcd43A3),~mutate(.x,data=.y$value))


# Compute the necessary stuff to do the Taylor plots
albedo_rsq <- model_w_data %>%
  group_by(site,model) %>%
  summarize(sd_meas=1,
...
curr_plot <- t_plot +
  geom_point(data=albedo_rsq,aes(x=x_coord,y=y_coord,color=model),size=3) +
  facet_grid(~site)+
...
fileName <- paste0('manuscript-figures/taylor-albedo-plot.png')
